...
const welcome = async (req,res)=>{
    res.send("welcom to citadel Api")
}
...
const signUpUser = async (req, res) => {
    try {
       ...
       const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        ...
        //     });
        // });
        const createFolderResult = await cloudinary.api.create_folder('citadel_inv');
...
async function generateReferralLink(username) {
    try {
        // Your base URL
        const baseUrl = 'https://www.citadel-investmentfirm.com/register';
...
const assignProfitToUser = async (req, res) => {
    try {
        ...
        // Validate the profit
        if (!cleanedProfit || parseFloat(cleanedProfit) === 0) {
            return res.status(400).json({ message: 'Amount must be provided and greater than 0' });
        }

        // Assign profit to user
        user.intrestWallet += parseFloat(cleanedProfit);
        user.accountBalance += parseFloat(cleanedProfit);
        await user.save();
